...
@OBSERVACIONFTR	   VARCHAR(2040) = NULL
WITH ENCRYPTION
AS 
...
BEGIN
   ...
   IF @PROC = 'CI'  --JEDM 20240209   COLOCO ESTE IF PQ ESTABA HACIEN UPDATE A CIT A SI NO VINIERA CON PROCEDENCIA CIT
	...
	BEGIN  
		...
		BEGIN
			PRINT 'ES DE CITAS'
			IF DBO.FNK_VALORVARIABLE('IMPRIME_DETPAQUETE')='SI' AND EXISTS(SELECT 1 FROM EPQ INNER JOIN CIT ON CIT.IDSERVICIO=EPQ.IDSERVICIO WHERE CIT.CONSECUTIVO=@NOAUT)
			BEGIN
				PRINT 'INSERTO PAQUETE Y DETALLES'
				INSERT INTO #FTRD1(CNSFTR, FECHA, DB_CR, AREAPRESTACION, UBICACION, VR_TOTAL,
						IMPUTACION, CCOSTO, PREFIJO, ANEXO, REFERENCIA, IDCIRUGIA, CANTIDAD,
						VALOR, VLR_SERVICI, VLR_COPAGOS, VLR_PAGCOMP, IDPROVEEDOR, NOADMISION,PROCEDENCIA,
						NOPRESTACION, NOITEM, AREAFUNCONT, N_FACTURA, SUBCCOSTO, PCOSTO, FECHAPREST, IDPLAN,IDIMPUESTO, IDCLASE,ITEM , VLRIMPUESTO,
				...
				SELECT @CNSFTR, GETDATE(), 'DB', CIT.IDAREA, NULL, 
					0,NULL, CIT.CCOSTO, SER.PREFIJO, SER.DESCSERVICIO, SER.IDSERVICIO, NULL,
					COALESCE(EPQ.CANTIDAD,1) CANTIDAD, 0, 0, 0,
					0, CIT.IDMEDICO, @NOAUT,'CI', @NOAUT, 1, NULL, 
					@NVOCONSEC, CIT.SUBCCOSTO, CIT.VALORTOTALCOS, CIT.FECHA, CIT.IDPLAN,
							CASE WHEN COALESCE(SER.IVA,'')='SER' THEN  SER.IDIMPUESTO ELSE NULL END,
							CASE WHEN COALESCE(SER.IVA,'')='SER' THEN  SER.IDCLASE ELSE NULL END,
							CASE WHEN COALESCE(SER.IVA,'')='SER' THEN  (SELECT ITEM FROM  DBO.FNK_VALOR_IMPUESTO(SER.IDIMPUESTO,SER.IDCLASE,GETDATE(), 0)) ELSE NULL END,
							CASE WHEN COALESCE(SER.IVA,'')='SER' THEN  0 ELSE NULL END ,
							CASE WHEN COALESCE(SER.IVA,'')='SER' THEN  (SELECT VALOR FROM  DBO.FNK_VALOR_IMPUESTO(SER.IDIMPUESTO,SER.IDCLASE,GETDATE(),0)) ELSE NULL END,
							CASE WHEN COALESCE(SER.IVA,'')='SER' THEN  CASE WHEN @BASE_IVA_SER='CONIVA' 
																			THEN  (SELECT 0 -ROUND(0/((VALOR+100)/100),0) 
																			FROM  DBO.FNK_VALOR_IMPUESTO(SER.IDIMPUESTO,SER.IDCLASE,GETDATE(),0))
																		ELSE
																		(SELECT ROUND(0*(VALOR/100),0) 
																		FROM  DBO.FNK_VALOR_IMPUESTO(SER.IDIMPUESTO,SER.IDCLASE,GETDATE(),0)) END ELSE NULL END,
							CASE WHEN COALESCE(SER.IVA,'')='SER' THEN  (SELECT CUENTA FROM  DBO.FNK_VALOR_IMPUESTO(SER.IDIMPUESTO,SER.IDCLASE,GETDATE(),0))  ELSE NULL END,
					  DBO.FNK_TRAE_TIPOCOPAGO(CIT.CONSECUTIVO,'CI')
				FROM   CIT INNER JOIN EPQ ON EPQ.IDSERVICIO = CIT.IDSERVICIO AND EPQ.IDPLAN = CIT.IDPLAN AND EPQ.IDPROVEEDOR = CIT.IDTERCEROCA
							INNER JOIN SER ON EPQ.IDSERVDET=SER.IDSERVICIO 
							LEFT JOIN TER ON TER.IDTERCERO = CIT.IDTERCEROCA
				WHERE  CIT.CONSECUTIVO   = @NOAUT
				AND    SER.PREFIJO       <> @PRE1
				AND    SER.PREFIJO       <> @PRE2
			...
			ELSE
			BEGIN	--VALORTOTAL
            IF EXISTS (SELECT 1 FROM CIT WHERE CONSECUTIVO = @NOAUT AND CIT.TIPOCITA = DBO.FNK_VALORVARIABLE('CIT_MULTI_SER')) AND EXISTS(SELECT 1 FROM CITSER WHERE CONSECUTIVO_CIT = @NOAUT)
            BEGIN
               INSERT INTO #FTRD1(CNSFTR, FECHA, DB_CR, AREAPRESTACION, UBICACION, VR_TOTAL,
						   IMPUTACION, CCOSTO, PREFIJO, ANEXO, REFERENCIA, IDCIRUGIA, CANTIDAD,
						   VALOR, VLR_SERVICI, VLR_COPAGOS, VLR_PAGCOMP, IDPROVEEDOR, NOADMISION,PROCEDENCIA,
				      ...
				      PIVA, VIVA,CUENTA_FIMPDV,TCOPAGO)
				   SELECT @CNSFTR, GETDATE(), 'DB', CIT.IDAREA, NULL, 
					   CASE WHEN @IDTERCEROCA = 'PARTN' THEN 1*CITSER.VALORTOTAL ELSE -- ADD: JREYES 20090810 CORREC:209
						   CASE WHEN @NODESCUENTACOPAGO = 1 THEN 1*CITSER.VALORTOTAL ELSE (1*CITSER.VALORTOTAL) - COALESCE(CITSER.VALOR_COPAGO,0) END END, 
					   NULL, CIT.CCOSTO, SER.PREFIJO, SER.DESCSERVICIO, SER.IDSERVICIO, NULL,
					   1 CANTIDAD, CITSER.VALORTOTAL, 1*CITSER.VALORTOTAL, CASE WHEN @NODESCUENTACOPAGO = 1 THEN 0 ELSE COALESCE(CITSER.VALOR_COPAGO,0) END,
					   0, CIT.IDMEDICO, @NOAUT,'CI', @NOAUT, 1, NULL, 
					   @NVOCONSEC, CIT.SUBCCOSTO, CIT.VALORTOTALCOS, CIT.FECHA, CIT.IDPLAN,
							   CASE WHEN COALESCE(SER.IVA,'')='SER' THEN  SER.IDIMPUESTO ELSE NULL END,
							   CASE WHEN COALESCE(SER.IVA,'')='SER' THEN  SER.IDCLASE ELSE NULL END,
							   CASE WHEN COALESCE(SER.IVA,'')='SER' THEN  (SELECT ITEM FROM  DBO.FNK_VALOR_IMPUESTO(SER.IDIMPUESTO,SER.IDCLASE,GETDATE(),(1*CITSER.VALORTOTAL))) ELSE NULL END,
							   CASE WHEN COALESCE(SER.IVA,'')='SER' THEN  (1*CITSER.VALORTOTAL) ELSE NULL END ,
							   CASE WHEN COALESCE(SER.IVA,'')='SER' THEN  (SELECT VALOR FROM  DBO.FNK_VALOR_IMPUESTO(SER.IDIMPUESTO,SER.IDCLASE,GETDATE(),(1*CITSER.VALORTOTAL))) ELSE NULL END,
							   CASE WHEN COALESCE(SER.IVA,'')='SER' THEN  CASE WHEN @BASE_IVA_SER='CONIVA' 
																			   THEN  (SELECT (1*CITSER.VALORTOTAL) -ROUND((1*CITSER.VALORTOTAL)/((VALOR+100)/100),0) 
																			   FROM  DBO.FNK_VALOR_IMPUESTO(SER.IDIMPUESTO,SER.IDCLASE,GETDATE(),(1*CITSER.VALORTOTAL)))
																		   ELSE
																		   (SELECT ROUND((1*CITSER.VALORTOTAL)*(VALOR/100),0) 
																		   FROM  DBO.FNK_VALOR_IMPUESTO(SER.IDIMPUESTO,SER.IDCLASE,GETDATE(),(1*CITSER.VALORTOTAL))) END ELSE NULL END,
							   CASE WHEN COALESCE(SER.IVA,'')='SER' THEN  (SELECT CUENTA FROM  DBO.FNK_VALOR_IMPUESTO(SER.IDIMPUESTO,SER.IDCLASE,GETDATE(),(1*CITSER.VALORTOTAL)))  ELSE NULL END,
					     DBO.FNK_TRAE_TIPOCOPAGO(CIT.CONSECUTIVO,'CI')
				   FROM   CIT 
                  INNER JOIN CITSER ON CIT.CONSECUTIVO = CITSER.CONSECUTIVO_CIT
                  INNER JOIN SER ON SER.IDSERVICIO = CITSER.IDSERVICIO 
						LEFT JOIN TER ON TER.IDTERCERO = CIT.IDTERCEROCA
				   WHERE  CIT.CONSECUTIVO   = @NOAUT
				   AND    SER.PREFIJO       <> @PRE1
				   AND    SER.PREFIJO       <> @PRE2
				   ...
				   AND    (CIT.FACTURADA=0 OR CIT.FACTURADA IS NULL)
				   AND    (TER.IDTERCERO  = @IDTERCEROCA OR (TER.IDTERCERO IS NULL AND @IDTERCEROF=@IDTERCEROCA)) 
               AND COALESCE(CITSER.VALORTOTAL,0)>0
            END
            ELSE
            BEGIN
				   INSERT INTO #FTRD1(CNSFTR, FECHA, DB_CR, AREAPRESTACION, UBICACION, VR_TOTAL,
      ...
      BEGIN
         ...
         CONTINUE
      END
      IF (SELECT SUM(VLR_SERVICI-VLR_COPAGOS) FROM #FTRD1)<=0 AND DBO.FNK_VALORVARIABLE('FACTURAPOS_ACTIVA')='SI'
      BEGIN 
            IF @PROC = 'CE'
            BEGIN
		...
		BEGIN
            ...
            PRINT 'ACAAAAAAAAAA'
				...
				SELECT @CNSFTR, @COMPANIA, 'C', CASE WHEN @TV='Contado' AND @EC=1 THEN (CASE @IDTERCEROCA WHEN dbo.FNK_VALORVARIABLE('IDTERCEROPARTICULAR') 
						...
						@PROC, 'I', COALESCE(@OBSERVACIONFTR,''), @TV, 0, 0, 0, 0, 0, 0, 0, 0, NULL, 0, 0, 0, 0, NULL, NULL, @USUARIO,
