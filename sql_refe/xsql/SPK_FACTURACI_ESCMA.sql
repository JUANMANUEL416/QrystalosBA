...
DECLARE @NuevoIdAfiliado VARCHAR(20)
BEGIN
   ...
   DECLARE CUR_FTR_CIT CURSOR FOR
     ...
     SELECT @VRDTO = SUM(CASE WHEN CIT.TIPODTO='P' THEN VALORTOTAL* COALESCE(DESCUENTO,0)/100 ELSE COALESCE(DESCUENTO,0)END )
      ...
      BEGIN
		 ...
		 BEGIN
            ...
            END
         END
		 PRINT 'TRARCNSSS  ' + COALESCE(@TRAECONSEC,'NO TRAIGO NADA') 
         ...
         /* CITSES_DESG_COPA=SI: prorratea VLR_COPAGOS entre lineas CIT de la misma sesion (IDAUTSES) en esta factura.
            Si el copago asignado supera VLR_SERVICI, se topea al valor del servicio.
            COLLATE DATABASE_DEFAULT: evita conflicto tempdb (#FTRD1) vs BD (CIT). */
         IF DBO.FNK_VALORVARIABLE('CITSES_DESG_COPA') COLLATE DATABASE_DEFAULT = N'SI' COLLATE DATABASE_DEFAULT
            AND EXISTS (
               SELECT 1
               FROM #FTRD1 F
               INNER JOIN CIT C ON C.CONSECUTIVO COLLATE DATABASE_DEFAULT = F.NOPRESTACION COLLATE DATABASE_DEFAULT
               WHERE F.PROCEDENCIA COLLATE DATABASE_DEFAULT = 'CI' COLLATE DATABASE_DEFAULT
         ...
         BEGIN
            ;WITH BaseSes AS (
               SELECT
                  ...
                  F.VLR_SERVICI,
                  C.IDAUTSES,
                  SUM(F.VLR_COPAGOS) OVER (PARTITION BY C.IDAUTSES) AS TotalCopa,
                  COUNT(*) OVER (PARTITION BY C.IDAUTSES) AS Cant,
                  ROW_NUMBER() OVER (PARTITION BY C.IDAUTSES ORDER BY F.N_CUOTA) AS Rn
               FROM #FTRD1 F
               INNER JOIN CIT C ON C.CONSECUTIVO COLLATE DATABASE_DEFAULT = F.NOPRESTACION COLLATE DATABASE_DEFAULT
               WHERE F.PROCEDENCIA COLLATE DATABASE_DEFAULT = 'CI' COLLATE DATABASE_DEFAULT
                 AND C.IDAUTSES IS NOT NULL
            ...
            Asignado AS (
               SELECT
                  ...
                  VLR_SERVICI,
                  CASE
                     WHEN Rn < Cant THEN ROUND(TotalCopa / NULLIF(Cant, 0), 0)
                     ELSE TotalCopa - (ROUND(TotalCopa / NULLIF(Cant, 0), 0) * (Cant - 1))
                  END AS CopaCalc
               FROM BaseSes
            )
            UPDATE F
            SET
               VLR_COPAGOS = CASE WHEN A.CopaCalc > F.VLR_SERVICI THEN F.VLR_SERVICI ELSE A.CopaCalc END,
               VR_TOTAL = F.VLR_SERVICI - CASE WHEN A.CopaCalc > F.VLR_SERVICI THEN F.VLR_SERVICI ELSE A.CopaCalc END
            FROM #FTRD1 F
            INNER JOIN Asignado A ON A.N_CUOTA = F.N_CUOTA
         END
   ...
   SELECT @CANTAFI=COUNT(DISTINCT IDAFILIADO) 
   ...
   BEGIN
		IF COALESCE(@CANTAFI, 0) > 1
		BEGIN
			-- Si hay más de un afiliado, se borra el campo para evitar inconsistencia
			UPDATE FTR
			SET IDAFILIADO = NULL
			WHERE N_FACTURA = @NVOCONSEC;
		END
		ELSE
		BEGIN
			-- Si hay un solo afiliado, intentamos encontrarlo y asignarlo
			SELECT TOP 1 @NuevoIdAfiliado = IDAFILIADO
			FROM (
				SELECT AUT.IDAFILIADO
				FROM AUTD 
				INNER JOIN AUT ON AUT.IDAUT = AUTD.IDAUT
				WHERE AUTD.N_FACTURA = @NVOCONSEC

				UNION ALL

				SELECT AUT.IDAFILIADO
				FROM AUT
				WHERE AUT.N_FACTURA = @NVOCONSEC

				UNION ALL

				SELECT CIT.IDAFILIADO
				FROM CIT
				WHERE CIT.N_FACTURA = @NVOCONSEC
			) AS REF
			WHERE REF.IDAFILIADO IS NOT NULL
			ORDER BY REF.IDAFILIADO DESC;

			UPDATE FTR
			SET IDAFILIADO = @NuevoIdAfiliado
			WHERE N_FACTURA = @NVOCONSEC;
		END
   END 
