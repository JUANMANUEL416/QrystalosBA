...
--
   ...
   DECLARE CUR_FTR_AUT CURSOR FOR
   ...
   WHERE  AUT.CNSFACT = @CNSRPDX AND (AUT.FACTURADA = 0 OR AUT.FACTURADA IS NULL OR AUT.FACTURADA=2) AND
	...
	-- Recalcular VALORTOTAL excluyendo items NO COBRABLES (NAREN)
	UPDATE AUT SET VALORTOTAL = (
		SELECT COALESCE(SUM(COALESCE(AUTD.VALOR,0) * COALESCE(AUTD.CANTIDAD,1)), 0)
		FROM AUTD
		WHERE AUTD.IDAUT = AUT.IDAUT
		AND COALESCE(AUTD.NOCOBRABLE,0) = 0  --  Solo items COBRABLES
		AND COALESCE(AUTD.FACTURADA,0) = 0   -- Solo items NO facturados
	)
	WHERE AUT.CNSFACT = @CNSRPDX
	AND AUT.MARCAFAC = 1
	AND COALESCE(AUT.FACTURABLE,0) = 1
	PRINT 'VALORTOTAL recalculado excluyendo items NO COBRABLES'

   OPEN CUR_FTR_AUT
   FETCH NEXT FROM CUR_FTR_AUT
   INTO @IDTERCEROCA
   ...
   BEGIN  
      ...
      AND    COALESCE(FACTURABLE,0)=1 
	  AND	 COALESCE(AUTD.NOCOBRABLE,0) = 0  --AGREGUE CAMBIO AQUI PARA QUE NO FACTURA LOS ITEM CON NOCOCABLE ==1 (NAREN)
      AND    ((COALESCE(VALORTOTAL,0)>0 AND @CLASEFACTURA <> 'CUE') OR (COALESCE(VALORTOTAL,0) = COALESCE(VALORTOTAL,0) AND @CLASEFACTURA = 'CUE'))
      ...
      BEGIN
		 ...
		 AND	COALESCE(AUTD.NOCOBRABLE,0) = 0  --- TAMBIEN AGREGO AQUI FILTRO DE NO COBRABLE (NAREN)
         AND   COALESCE(FACTURABLE,0)=1 
         AND   ((COALESCE(VALORTOTAL,0)>0 AND @CLASEFACTURA <> 'CUE') OR (COALESCE(VALORTOTAL,0) = COALESCE(VALORTOTAL,0) AND @CLASEFACTURA = 'CUE'))
         AND    COALESCE(AUT.DESCUENTO,0)<CASE WHEN AUT.TIPODTO='P' THEN 100 ELSE CASE WHEN @CLASEFACTURA <> 'CUE' THEN AUT.VALORTOTAL ELSE 999999999 END  END
         AND    AUT.IDAFILIADO = CASE @CLASEFACTURA WHEN 'CUE' THEN @IDTERCEROCA1 ELSE AUT.IDAFILIADO END
   ...
   SELECT @CANTAFI =COUNT (DISTINCT IDAFILIADO) FROM ( SELECT DISTINCT   IDAFILIADO  FROM AUTD INNER JOIN AUT ON AUT.IDAUT=AUTD.IDAUT WHERE AUTD.N_FACTURA=@NVOCONSEC) Z 
   ...
   ELSE
   BEGIN
   PRINT 'INGRESO SPK_FACTURACI_ESCMA  ' + COALESCE(@CNSRPDX, 'VENGO VACIO') + '@CNSCONSEC  ' +  @NVOCONSEC + 'CNSFTR  ' + @CNSFTR
    --  CLOSE CUR_FTR_AUT
