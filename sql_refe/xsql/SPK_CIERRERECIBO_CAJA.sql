...
BEGIN
   ...
   BEGIN   
      ...
      IF DBO.FNK_VALORVARIABLE('FACTURAPOS_ACTIVA')='SI' 
         AND EXISTS(
            SELECT * FROM FCJ WITH (NOLOCK)
            WHERE CODCAJA=@CODCAJA AND CNSFACJ=@CNSFACJ
              AND ESTADO='P'
              AND (
                  (
                      COALESCE(IDPLAN,'') NOT IN(SELECT DATO FROM USVGS WHERE IDVARIABLE LIKE'IDPLANPART%')
                      AND PROCEDENCIA<>'CAJA'
                  )
                  OR (
                      PROCEDENCIA='CAJA'
                      AND EXISTS(
                          SELECT 1 FROM FCJD D WITH (NOLOCK)
                          INNER JOIN MOCC M ON D.CONCEPTO=M.CODIGOCPCJ
      ...
      BEGIN
         ...
         BEGIN
            ...
            IF EXISTS(SELECT * FROM CIT WHERE CONSECUTIVO=@NOADMISION AND VALORTOTAL<=VALORCOPAGO AND IDTERCEROCA<>DBO.FNK_VALORVARIABLE('IDTERCEROPARTICULAR')  AND COALESCE(IDPLAN,'') NOT IN(SELECT DATO FROM USVGS WHERE IDVARIABLE LIKE'IDPLANPART%') )
            BEGIN
               PRINT 'VOY A GENERAR LA FACTURA'
               EXEC SPK_CREA_FACTURA_POS_CAJA @CNSFACJ,@CODCAJA,@COMPANIA,@SEDE,@USUARIO,@SYS_COMPUTERNAME,1,@ESPAGARE
            END
            IF EXISTS(SELECT * FROM AUT WHERE NOAUT=@NOADMISION AND VALORTOTAL<=VALORCOPAGO AND IDTERCEROCA<>DBO.FNK_VALORVARIABLE('IDTERCEROPARTICULAR')  AND COALESCE(IDPLAN,'') NOT IN(SELECT DATO FROM USVGS WHERE IDVARIABLE LIKE'IDPLANPART%'))
            BEGIN
               PRINT 'VOY A GENERAR LA FACTURA'
               EXEC SPK_CREA_FACTURA_POS_CAJA @CNSFACJ,@CODCAJA,@COMPANIA,@SEDE,@USUARIO,@SYS_COMPUTERNAME,1,@ESPAGARE
            END
   ...
   ELSE
    ...
    --     END
    --  END
   /* Traslado banco a caja: validar caja destino abierta antes de movimientos bancarios o cierre del recibo */
   IF @CLASE_FAC = 'PAGO' AND COALESCE(@ESTADO,'P') <> 'A'
      AND @TIPOEGRESO = @TRASACAJA
   BEGIN
      IF NOT EXISTS (
         SELECT 1 FROM CAJ
         WHERE CODCAJA = @IDTERCERO AND COALESCE(ABIERTA, 0) = 1
      ) 
      BEGIN
         RAISERROR(N'La caja destino del traslado banco a caja no está abierta. No se puede cerrar el recibo.', 16, 1)
         RETURN
      END
   END
...
--query2         
            ...
            -- Se excluyen PCJ con TARCRED=1 (tarjeta de credito) para que NO generen BMOV;
            -- esos pagos se manejan en la tabla BCTTAR (ver bloque al final del SP).

          PRINT 'REVISO SI TENGO ALGUN PAGO A TARJETA CREDITO'
          IF EXISTS(SELECT * FROM FCJD INNER JOIN FCXPP ON FCJD.IDSERVICIO=FCXPP.CNSFCXPP
                                        INNER JOIN FCXP ON FCXPP.CNSFCXP=FCXP.CNSFCXP
                    WHERE FCJD.CODCAJA=@CODCAJA
                    AND FCJD.CNSFACJ=@CNSFACJ
                    AND LEFT(FCXP.NOREFERENCIA,7)='TARCRED'
                    AND COALESCE(FCXP.TARCRED,'')<>''
                    )
          BEGIN
            DECLARE @TARCRED VARCHAR(255)
            DECLARE @ABONOTC DECIMAL(14,2)
            DECLARE @CNSFCXPTC VARCHAR(20)
            DECLARE @VLR_CUOTAS DECIMAL(14,2)
              DECLARE PAGOTC_CURSOR CURSOR FOR
              SELECT FCXP.CNSFCXP, FCXP.TARCRED,FCXPP.ABONO FROM FCJD INNER JOIN FCXPP ON FCJD.IDSERVICIO=FCXPP.CNSFCXPP
                                  INNER JOIN FCXP ON FCXPP.CNSFCXP=FCXP.CNSFCXP
              WHERE FCJD.CODCAJA=@CODCAJA
              AND FCJD.CNSFACJ=@CNSFACJ
              AND LEFT(FCXP.NOREFERENCIA,7)='TARCRED'
              AND COALESCE(FCXP.TARCRED,'')<>''
              OPEN PAGOTC_CURSOR    
              FETCH NEXT FROM PAGOTC_CURSOR    
              INTO @CNSFCXPTC,@TARCRED,@ABONOTC
              WHILE @@FETCH_STATUS = 0    
              BEGIN
                PRINT 'Aplicando pago Tarjeta de Credito.'
                SELECT @BANCO2 = BANCO, @SUCURSAL2 = SUCURSAL, @CTA_BCO2 = CTA_BCO
                FROM OPENJSON(@TARCRED)WITH(
                BANCO  VARCHAR(20)   '$.BANCO',
                SUCURSAL  VARCHAR(20)   '$.SUCURSAL',
                CTA_BCO  VARCHAR(20)   '$.CTA_BCO'
                )
                SELECT @VLR_CUOTAS=VLR_NETO
                FROM FCXPD WHERE CNSFCXP=@CNSFCXPTC
                AND IDSERVICIO='TC-001'

                SELECT @ITEMBMOV = COALESCE(MAX(BMOV.ITEM),0)+1
                FROM BMOV
                ...
                SELECT @BANCO2, @SUCURSAL2, @CTA_BCO2, @ITEMBMOV , @FECHA,
                      @CNSFACJ, NULL, DBO.FNK_VALORVARIABLE('IDOFINCAJADB'), 0,'Abonos y Pagos Tarjeta de Credito',
                      @USUARIO, @SYS_COMPUTERNAME, 0, 0, NULL, 'CAJA', 'C', 1, @NROCOMPROBANTE,
                      @CODCAJA, @CNSFACJ
                ...
                SELECT @BANCO2, @SUCURSAL2, @CTA_BCO2, @ITEMBMOV, @RENGLON, 
                        @TIPOPAGO, @BANCO, @NODOCUMENTO,  @VLR_CUOTAS, 'CAJA', @CODCAJA, @CNSFACJ, @IDTERCERO

                EXEC SPK_TOT_BMOV @BANCO2, @SUCURSAL2, @CTA_BCO2, @ITEMBMOV 
                EXEC SPK_OK_BMOV @BANCO2, @SUCURSAL2, @CTA_BCO2, @ITEMBMOV,@USUARIO,@SYS_COMPUTERNAME,@COMPANIA,@SEDE,@NROCOMPROBANTE

                FETCH NEXT FROM PAGOTC_CURSOR    
                INTO @CNSFCXPTC,@TARCRED,@ABONOTC
              END
              CLOSE PAGOTC_CURSOR
              DEALLOCATE PAGOTC_CURSOR
                     ...
                     BEGIN
                        ...
                        BEGIN  
                           ...
                           BEGIN
                              ...
                              BEGIN
                                 EXEC SPK_OK_BMOV  @BANCO, @SUCURSAL, @CTA_BCO, @ITEMBMOV,@USUARIO,@SYS_COMPUTERNAME,@COMPANIA,@SEDE,@NROCOMPROBANTE
                                 SELECT @NROCOMPROBANTE = COALESCE(B.NROCOMPROBANTE, @NROCOMPROBANTE)
                                 FROM BMOV B
                                 WHERE B.BANCO = @BANCO
                                   AND B.SUCURSAL = @SUCURSAL
                                   AND B.CTA_BCO = @CTA_BCO
                                   AND B.ITEM = @ITEMBMOV
                              END
                              ELSE
                              BEGIN
                        ...
                        BEGIN
                           ...
                           SELECT @NROCOMPROBANTE = COALESCE(MAX(B.NROCOMPROBANTE), @NROCOMPROBANTE)
                           FROM BMOV B
                           WHERE B.CODCAJA = @CODCAJA
                             AND B.CNSFACJ = @CNSFACJ
                             AND COALESCE(B.NROCOMPROBANTE,'') <> ''
                           ...
                           SELECT @NVOCONSEC2, @CAJAREP, 'COBRO', NOADMISION, NOPRESTACION, IDPLAN, VALORTOTAL, COMPANIA, @CODCAJA, CERRADA, 
                                  FECHA, 'EXTERNO', PROCEDENCIA, N_FACTURA, @CODCAJA, NOMBRECLIENTE, @CNSACJ, N_RECIBO, ESTADO, IMPRESO, 1,
                                  1, @NROCOMPROBANTE, AREAPRESTACION, CCOSTO, SUBCCOSTO, CLASER, IDCAUSALANUL, USUARIOANUL, 
                                  SYS_COMPUTERNAMEANUL, FECHAANU, TCNSFACJ, IDENTIFICADOR, SYS_COMPUTERNAME, OBSERVACION, RECAUDO, ENPAGOS,
                                  TIPOEGRESO, IMPRESOTRAS, BANCO, SUCURSAL, CTA_BCO, USUARIO, 3
                           FROM   FCJ
                           WHERE  CNSFACJ = @CNSFACJ AND CODCAJA = @CODCAJA
						   ...
						   UPDATE FCJ SET IDTERCERO = DBO.FNK_VALORVARIABLE('IDTERCEROINSTALADO'),
                                          CERRADA = 1,
                                          CONTABILIZADA = CASE WHEN COALESCE(@NROCOMPROBANTE,'')<>'' THEN 1 ELSE CONTABILIZADA END,
                                          MARCACONT = CASE WHEN COALESCE(@NROCOMPROBANTE,'')<>'' THEN 1 ELSE MARCACONT END,
                                          NROCOMPROBANTE = CASE WHEN COALESCE(@NROCOMPROBANTE,'')<>'' THEN @NROCOMPROBANTE ELSE NROCOMPROBANTE END
                           WHERE CNSFACJ=@NVOCONSEC2 AND CODCAJA=@CAJAREP

                           IF COALESCE(@NROCOMPROBANTE,'')<>''
                           BEGIN
                              UPDATE FCJ SET CONTABILIZADA = 1,
                                               MARCACONT = 1,
         ...
         */
         ...
         -- =====================================================================
         -- GENERACION DE CUOTAS DE TARJETA DE CREDITO (BCTTAR)
         -- ---------------------------------------------------------------------
         -- Si el recibo cerrado tiene PCJ con TARCRED=1 (pago con tarjeta credito
         -- institucional), se generan NCUOTAS filas en BCTTAR (una por cuota) y
         -- se descuenta BCT.CUPODISP por el valor total de la compra.
         -- Estos PCJ NO generan BMOV (fueron excluidos del #TPCJ mas arriba).
         -- =====================================================================
         IF EXISTS (
            SELECT 1 FROM PCJ
            WHERE CODCAJA = @CODCAJA AND CNSFACJ = @CNSFACJ
              AND COALESCE(TARCRED,0) = 1 AND COALESCE(NCUOTAS,0) > 0
         )
         BEGIN
            DECLARE @TC_CNSPCJ   VARCHAR(20),
                    @TC_BANCO    VARCHAR(3),
                    @TC_SUCURSAL VARCHAR(2),
                    @TC_CTABCO   VARCHAR(40),
                    @TC_VALOR    DECIMAL(14,2),
                    @TC_NCUOTAS  SMALLINT,
                    @TC_I        SMALLINT,
                    @TC_VALCUOTA DECIMAL(14,2),
                    @TC_ULTIMA   DECIMAL(14,2),
                    @TC_FVENCE   DATETIME

            DECLARE TARCRED_CUR CURSOR LOCAL FAST_FORWARD FOR
               SELECT CNSPCJ, BANCO, SUCURSAL, CTA_BCO, VALOR, NCUOTAS
               FROM   PCJ
               WHERE  CODCAJA = @CODCAJA AND CNSFACJ = @CNSFACJ
                 AND  COALESCE(TARCRED,0) = 1 AND COALESCE(NCUOTAS,0) > 0

            OPEN TARCRED_CUR
            FETCH NEXT FROM TARCRED_CUR
               INTO @TC_CNSPCJ, @TC_BANCO, @TC_SUCURSAL, @TC_CTABCO, @TC_VALOR, @TC_NCUOTAS

            WHILE @@FETCH_STATUS = 0
            BEGIN
               SET @TC_VALCUOTA = ROUND(@TC_VALOR / @TC_NCUOTAS, 2)
               SET @TC_ULTIMA   = @TC_VALOR - (@TC_VALCUOTA * (@TC_NCUOTAS - 1))
               SET @TC_I = 1

               WHILE @TC_I <= @TC_NCUOTAS
               BEGIN
                  SET @TC_FVENCE = DATEADD(MONTH, @TC_I, @FECHA)

                  INSERT INTO BCTTAR (BANCO, SUCURSAL, CTA_BCO, CODCAJA, CNSFACJ, CNSPCJ,
                                      NROCUOTA, NCUOTAS, F_COMPRA, F_VENCE,
                                      ANOCUOTA, MESCUOTA, VALORCOMPRA, VALORCUOTA,
                                      ESTADO, USUARIO)
                  VALUES (@TC_BANCO, @TC_SUCURSAL, @TC_CTABCO, @CODCAJA, @CNSFACJ, @TC_CNSPCJ,
                          @TC_I, @TC_NCUOTAS, @FECHA, @TC_FVENCE,
                          DATEPART(YEAR, @TC_FVENCE), DATEPART(MONTH, @TC_FVENCE),
                          @TC_VALOR,
                          CASE WHEN @TC_I = @TC_NCUOTAS THEN @TC_ULTIMA ELSE @TC_VALCUOTA END,
                          'P', @USUARIO)

                  SET @TC_I = @TC_I + 1
               END

               -- Descontar cupo disponible de la tarjeta
               --UPDATE BCT
               --   SET CUPODISP = COALESCE(CUPODISP,0) - @TC_VALOR
               --WHERE BANCO = @TC_BANCO
               --  AND SUCURSAL = @TC_SUCURSAL
               --  AND CTA_BCO  = @TC_CTABCO
