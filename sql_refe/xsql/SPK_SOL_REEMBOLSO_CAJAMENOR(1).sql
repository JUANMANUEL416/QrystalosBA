CREATE OR ALTER PROCEDURE DBO.SPK_SOL_REEMBOLSO_CAJAMENOR
@CODCAJA          VARCHAR(4),
...
DECLARE @VALOR_BRUTO    DECIMAL(14,2)
...
PRINT 'REEMBOLSO'
   ...
   EXEC DBO.SPK_CIERRE_DE_CAJA @CODCAJA, @SYS_COMPUTERNAME, @SEDE, @COMPANIA, @FECHA_CIERRE
   ...
   SELECT @N_FACTURA = @CODCAJA + CAST(YEAR(ISNULL(@FECHA_CIERRE,@FECHA)) AS VARCHAR(4)) + 
                                  CASE WHEN MONTH(@FECHA) > 9 THEN CAST(MONTH(ISNULL(@FECHA_CIERRE,@FECHA)) AS VARCHAR(2)) 
                                       ELSE '0' + LTRIM(RTRIM(CAST(MONTH(ISNULL(@FECHA_CIERRE,@FECHA)) AS VARCHAR(2))))
                                  END +
                                  CASE WHEN DAY(ISNULL(@FECHA_CIERRE,@FECHA)) > 9 THEN CAST(DAY(ISNULL(@FECHA_CIERRE,@FECHA)) AS VARCHAR(2))
                                       ELSE '0' + LTRIM(RTRIM(CAST(DAY(ISNULL(@FECHA_CIERRE,@FECHA)) AS VARCHAR(2))))
                                  END + '_' + 
                                  CASE WHEN DATEPART(HH,ISNULL(@FECHA_CIERRE,@FECHA)) > 9 THEN CAST(DATEPART(HH,ISNULL(@FECHA_CIERRE,@FECHA)) AS VARCHAR(2))
                                       ELSE '0' + LTRIM(RTRIM(CAST(DATEPART(HH,ISNULL(@FECHA_CIERRE,@FECHA)) AS VARCHAR(2)))) 
                                  END + 
                                  CASE WHEN DATEPART(MI, ISNULL(@FECHA_CIERRE,@FECHA)) > 9 THEN CAST(DATEPART(MI, ISNULL(@FECHA_CIERRE,@FECHA)) AS VARCHAR(2))
                                       ELSE '0' + LTRIM(RTRIM(CAST(DATEPART(MI, ISNULL(@FECHA_CIERRE,@FECHA)) AS VARCHAR(2))))
                                  END     
   ...
   SELECT @NVOCONSEC, ISNULL(@FECHA_CIERRE,@FECHA), NULL, @RESPONSABLE, @N_FACTURA /*@CODCAJA*/, ISNULL(@FECHA_CIERRE,@FECHA),ISNULL(@FECHA_CIERRE,@FECHA), 0, 'N', NULL, @USUARIO, 
          ...
          YEAR(ISNULL(@FECHA_CIERRE,@FECHA)), MONTH(ISNULL(@FECHA_CIERRE,@FECHA)), NULL, NULL, @CIUDAD,0, @CODCAJA
   ...
   SELECT @COMPANIA, @NVOCONSEC1, 'CAJAMENOR', @NVOCONSEC2, YEAR(ISNULL(@FECHA_CIERRE,@FECHA)), MONTH(ISNULL(@FECHA_CIERRE,@FECHA)), ISNULL(@FECHA_CIERRE,@FECHA), 0, 0, @CNSCMREE, @N_FACTURA,@CODCAJA,
          @USUARIO,ISNULL(@FECHA_CIERRE,@FECHA), NULL, 'REEMBOLSO CAJA MENOR No. '+@CODCAJA, @SEDE, 0, NULL, @COMPROBANTECM, @RESPONSABLE, NULL, NULL, 0, NULL, NULL, 0, NULL
   ...
   SELECT FCJD.CNSFACJ, FCJD.ITEM, CPCJ.CUENTA, ISNULL(FCJD.VALORTOTAL,0), ISNULL(FCJD.VALOR_BRUTO,0), FCJD.CONCEPTO, FCJD.AREAFUNCIONAL, ISNULL(FCJD.ESIMPTOGEN,0), -- MOD.JQUIROGA 20090808 (ISNULL(FCJD.ESIMPTOGEN,0))
          FCJD.CCOSTO, --CAMBIO DE FCJ A FCJD
		  FCJ.IDTERCERO
   FROM   FCJD INNER JOIN FCJ  ON FCJD.CODCAJA  = FCJ.CODCAJA AND FCJD.CNSFACJ = FCJ.CNSFACJ
   ...
   AND NOT EXISTS(SELECT 1 FROM FCXPP INNER JOIN FCXP ON FCXP.CNSFCXP=FCXPP.CNSFCXP 
					WHERE CODCAJA=FCJ.CODCAJA AND CNSFACJ=FCJ.CNSFACJ AND FCXP.PROCEDENCIA<>'CAJA')
   UNION ALL 
   SELECT FCJD.CNSFACJ, FCJD.ITEM, CPCJ.CUENTA, ISNULL(FCJD.VALORTOTAL,0),ISNULL(FCJD.VALOR_BRUTO,0), FCJD.CONCEPTO, FCJD.AREAFUNCIONAL, ISNULL(FCJD.ESIMPTOGEN,0), -- MOD.JQUIROGA 20090808 (ISNULL(FCJD.ESIMPTOGEN,0))
   ...
   INTO @CNSFACJ, @ITEMFCJD, @CUENTACM, @VALORTOTAL, @VALOR_BRUTO, @CONCEPTO, @IDAREA, @OK, @CCOSTO, @IDTERCERO 
   ...
   PRINT @CCOSTO
      ...
      IF @OK = 1
      BEGIN
         SELECT @VALORI = @VALORI + @VALOR_BRUTO
      END
      ...
      BEGIN
         ...
         SELECT @VALOR_BRUTO = @VALOR_BRUTO - ISNULL(SUM(VALOR_BRUTO),0)
         FROM   FCJD 
         WHERE  CNSFACJ    = @CNSFACJ
         AND    ITEM       = @ITEMFCJD
         AND    ESIMPTOGEN = 1

		 IF COALESCE(@VALOR_BRUTO, 0) = 0
		 BEGIN
			SELECT @VALOR_BRUTO = @VALORTOTAL
		 END
         
         SELECT @VALORT = @VALORT + @VALOR_BRUTO      
         ...
         SELECT @NVOCONSEC, @ITEM, @CONCEPTO, 1, @VALOR_BRUTO, 'P', @CNSFACJ, @ITEMFCJD, CASE @OK WHEN 1 THEN '1' ELSE '0' END,
                0, 0, @VALOR_BRUTO, NULL, NULL, NULL, 
      ...
      SELECT @NVOCONSEC1, @COMPANIA, CASE @OK WHEN 1 THEN 'CR' ELSE 'DB' END, @CUENTACM, @IDTERCERO, @CCOSTO,
             @DETALLE, @VALOR_BRUTO, @CNSFACJ, @ITEMFCJD, NULL, NULL, NULL, NULL, 'Movimiento', NULL, NULL, NULL, @USUARIO, ISNULL(@FECHA_CIERRE,@FECHA), 
             @CONCEPTO, @IDAREA, 'S', '0', 'REEMBOLSO_CM', NULL, NULL, 'CAJAMENOR', @CNSFACJ, 0, NULL, NULL, NULL, NULL, NULL,
             NULL, NULL, 0 
  

	  --PARTE DE NUEVOS IMPUESTOS EN CAJA MENOR
	  IF EXISTS (SELECT 1 FROM FCJDIMP WHERE CODCAJA=@CODCAJA AND CNSFACJ = @CNSFACJ AND ITEM = @ITEMFCJD)
	  BEGIN
	  	  INSERT INTO MCHE( NROCOMPROBANTE, COMPANIA, TIPO, CUENTA, IDTERCERO, CCOSTO, DETALLE, VALOR, REFERENCIA1, REFERENCIA2, 
	  					   REFERENCIA3, N_FACTURA, F_FACTURAREF, F_VENCE, GENERA, CLASE_GENERA, CNSPAGO, ITEMREF, USUARIO, FECHA, PREFIJO,
	  					   IDAREA, CLASECONT, ESTADO, PROCESO, VALOR_PORCIMP, BASE_IMP, PROCEDENCIA, REFERENCIA_PRO, CONCILIADO, CNSFCXCXP,
	  					   CODUNG, CODPRG, ERROR, IDOPERACION, FECHACONCILIACION, IDPROVEEDOR, ENPRESUPUESTO )
	  	  SELECT @NVOCONSEC1, @COMPANIA, CASE TIPO WHEN 'CREDITO' THEN 'CR' ELSE 'DB' END, CUENTA, @IDTERCERO, @CCOSTO,
	  			 @DETALLE, VALOR, IDIMPUESTO, IDCLASE, ITEM_FIMPDV, NULL, NULL, NULL, 'Movimiento', NULL, NULL, NULL, @USUARIO,ISNULL(@FECHA_CIERRE,@FECHA), 
	  			 @CONCEPTO, @IDAREA, 'S', '0', 'FCJD_IMPUESTO', PORCENTAJE, BASE, 'CAJAMENOR', CNSFACJ, 0, NULL, NULL, NULL, NULL, NULL,
	  			 NULL, NULL, 0 
		  FROM FCJDIMP WHERE CODCAJA=@CODCAJA AND CNSFACJ = @CNSFACJ AND ITEM = @ITEMFCJD
	  END
   ...
   SELECT @NVOCONSEC1, @COMPANIA,  'CR', @CUECREDITO, @RESPONSABLE, @CCOSTO, 'CXP', @VALORT - @VALORI, @CNSCMREE, NULL, NULL, NULL,
          NULL, NULL, 'Movimiento', NULL, NULL, NULL, @USUARIO, ISNULL(@FECHA_CIERRE,@FECHA), NULL, NULL, 'S', '0', 'REEMBOLSO_CM', NULL, NULL, 'CAJAMENOR', 
   ...
   SELECT @RESPONSABLE, @N_FACTURA , 'CXP', ISNULL(@FECHA_CIERRE,@FECHA), ISNULL(@FECHA_CIERRE,@FECHA), 0, 'P'
