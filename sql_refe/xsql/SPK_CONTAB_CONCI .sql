IF EXISTS(SELECT NAME FROM SYSOBJECTS WHERE NAME='SPK_CONTAB_CONCI' AND XTYPE='P')
BEGIN
   DROP PROCEDURE SPK_CONTAB_CONCI 
END

GO
CREATE PROCEDURE DBO.SPK_CONTAB_CONCI 
@CNSCONCI         VARCHAR(20), 
@USUARIO          VARCHAR(12),
...
DECLARE @FNOT_FECHACONCILIA VARCHAR(10)
DECLARE @FECHATRA         DATETIME
DECLARE @SI               INT
BEGIN   ---SELECT * FROM FCONCID 
   SELECT   @FNOT_FECHACONCILIA=DBO.FNK_VALORVARIABLE('FNOT_FECHACONCILIA')
   SELECT   @ANO = CASE @FNOT_FECHACONCILIA WHEN 'CONCILIACION' THEN YEAR(FECHACONC) ELSE YEAR(GETDATE()) END,
            @MES = CASE @FNOT_FECHACONCILIA WHEN 'CONCILIACION' THEN MONTH(FECHACONC) ELSE MONTH(GETDATE()) END,
			@DIA = CASE @FNOT_FECHACONCILIA WHEN 'CONCILIACION' THEN DAY(FECHACONC) ELSE DAY(GETDATE()) END
   FROM FCONCI
   ...
   BEGIN
      ...
      SELECT @NROCOMPROBANTE, @COMPANIA, 'CONCI', @NOREFERENCIA, @ANO, @MES, 
             CAST(STR(DAY(@DIA))+'/'+LTRIM(STR(@MES))+'/'+@ANO AS DATETIME), 0, 0, IDFCONCI, '', @USUARIO, 
   ...
   SELECT @NROCOMPROBANTE, @COMPANIA, 'DB', D.CUENTACG,A.IDTERCERO,'' AS CCOSTO, 
          'CONCILIACION  DE GLOSA FACTURA: '+B.N_FACTURA, B.VLRRECUPERAR, A.IDFCONCI, B.CNSGLO, NULL, 
   ...
   INSERT INTO MCHE(NROCOMPROBANTE, COMPANIA, TIPO, CUENTA, IDTERCERO, CCOSTO, DETALLE, VALOR ,REFERENCIA1,
          	        REFERENCIA2, ITEMREF, USUARIO, FECHA, IDAREA, PREFIJO, ESTADO, PROCESO, GENERA, 
                   CLASECONT, N_FACTURA, F_FACTURAREF, F_VENCE, PROCEDENCIA, REFERENCIA_PRO, CODUNG, CODPRG)
   SELECT @NROCOMPROBANTE, @COMPANIA, 'CR', CASE WHEN B.TIPO='F' THEN D.CUENTARAD
