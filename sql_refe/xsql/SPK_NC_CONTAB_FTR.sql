...
DECLARE @CODCAJA VARCHAR(4), @CNSFACJ VARCHAR(20), @ESCOPAGOPOS BIT
...
BEGIN
	...
	SET DATEFORMAT dmy
   -- bugfix/CESPER87 -- 2026-09-05 -- Copago: hay FCJ.N_FACTURAPOS; no es particular (N_FACTURA)
   SET @ESCOPAGOPOS = 0
   IF EXISTS (
      SELECT 1 FROM FCJ WITH (NOLOCK)
      WHERE N_FACTURAPOS = @N_FACTURA AND ESTADO = 'P'
   )
   BEGIN
      SET @ESCOPAGOPOS = 1
   END
   ...
   BEGIN
      ...
      RETURN
   END
   /*SOLO CONTABILIZA FACTURAS ENVIADAS A LA DIAN*/
   IF DBO.FNK_VALORVARIABLE('CONTAB_FTR_POSTDIAN')='SI'
   BEGIN
      IF NOT EXISTS(SELECT * FROM FTR WHERE N_FACTURA=@N_FACTURA AND FACTE=2)
      BEGIN
         PRINT 'Factura no se ha enviado a la Dian, no se puede Contabilizar, me regreso'
         RETURN
      END
      --Validaci�n de Valores 
      SELECT @CNSDOCUMENTO=CNSFCT FROM FTR WHERE N_fACTURA=@N_FACTURA
      PRINT '@CNSDOCUMENTO ='+@CNSDOCUMENTO
      SELECT TOP 1 @XML=xml_solicitud FROM FDIANR 
      WHERE CNSDOCUMENTO=@CNSDOCUMENTO 
      ...
      SELECT
          ...
          PRINT '@VLRSERVICIOSDIAN ='+STR(@VLRSERVICIOSDIAN)
          PRINT '@VLRCOPAGODIAN ='+STR(@VLRCOPAGODIAN)
          PRINT '@VLR_TOTALDIAN ='+STR(@VLR_TOTALDIAN)
      IF NOT EXISTS(SELECT * FROM FTR WHERE N_FACTURA=@N_FACTURA 
                    AND ROUND(VR_TOTAL,0)=ROUND(@VLR_TOTALDIAN,0) 
                    AND ROUND(VALORSERVICIOS,2)-CASE WHEN COALESCE(VIVA,0)>0 THEN VIVA ELSE 0 END=ROUND(@VLRSERVICIOSDIAN,2) 
                    AND @VLRCOPAGODIAN=COALESCE(CASE WHEN COPAPROPIO=1 THEN CP_VLR_COPAGOS ELSE VALORCOPAGO END,0) ) 
      BEGIN
         DECLARE @MSG VARCHAR(MAX)

         SET @MSG = 'Inconsistencias en Valores. ' +
                    'Factura: ' + @N_FACTURA +
                    ', Servicios DIAN=' + CAST(COALESCE(@VLRSERVICIOSDIAN,0) AS VARCHAR(20)) +
                    ', Copago DIAN=' + CAST(COALESCE(@VLRCOPAGODIAN,0) AS VARCHAR(20)) +
                    ', Total DIAN=' + CAST(COALESCE(@VLR_TOTALDIAN,0) AS VARCHAR(20))
         PRINT @MSG
         RAISERROR(@MSG,16,1)
         RETURN
      END
   ...
   BEGIN
      ...
      /* COPAPROPIO: el copago se digita a mano y no depende de lo recaudado.
         No se genera @DIFECOPAGOS; la contabilizaci�n sigue l�gica de capita sin p�rdida (CxC = VR_TOTAL). */
      SELECT @COPAPROPIO = COALESCE(COPAPROPIO,0),
             @DIFECOPAGOS = 0
      FROM FTR WHERE N_FACTURA = @N_FACTURA
      PRINT '@COPAPROPIO ='+STR(@COPAPROPIO)
      PRINT '@DIFECOPAGOS ='+STR(@DIFECOPAGOS)
      SELECT @PGP=2
      IF NOT EXISTS(SELECT * FROM FTRDC WHERE CNSFTR=@CNSFTR)
      BEGIN
         IF NOT EXISTS( SELECT * FROM PPT INNER JOIN FTR ON PPT.IDTERCERO=FTR.IDTERCERO AND FTR.IDPLAN=PPT.IDPLAN AND PPT.CAPITADO=1 AND COALESCE(FTR.TIPOCAP,0)=1 AND FTR.CNSFCT=@CNSFTR)
         BEGIN
            UPDATE FTR SET CAPITADA=0 WHERE CNSFCT=@CNSFTR
         END
      END
   END
   ELSE
   BEGIN
      UPDATE FTRD SET PREFIJO=(SELECT TOP 1 PREFIJO FROM PRE WHERE PREFIJO IN(SELECT WORD FROM  DBO.FNK_EXPLODE(DBO.FNK_VALORVARIABLE('PREFIJOCITAS'),',')))
      WHERE CNSFTR=@CNSFTR
      AND NOT EXISTS(SELECT * FROM PRE WHERE FTRD.PREFIJO=PRE.PREFIJO)
   END
   ...
   BEGIN
      PRINT 'Comprobante ya esta en contabilidad, verifico que sea la misma factura'
      IF EXISTS(SELECT * FROM MCP WHERE PROCEDENCIA='FACTURA' AND  NOREFERENCIA=@N_FACTURA AND COALESCE(ANULADO,0)<>1)
      BEGIN
		   PRINT 'ME REGRESO POSIBLE DUPLICIDAD POR FAVOR REVISAR  PARO 1'  --20230215 OSOLANO
      ...
      END
   END
   IF ISNULL(@NROCOMPROBANTE,'') <> '' AND NOT EXISTS(SELECT * FROM MCPE WHERE NROCOMPROBANTE=@NROCOMPROBANTE)
   BEGIN
      PRINT 'se retira el nro de comprobante para asignar uno nuevo'
   ...
   BEGIN
      SELECT @PROCEDENCIA    = PROCEDENCIA, @NOREFERENCIA = NOREFERENCIA, @TIPOFAC = TIPOFAC, @OK = CONTABILIZADA,
             ...
             @VALORCOPAGO	  = CASE WHEN COALESCE(CAPITADA,0)=0 THEN COALESCE(VALORCOPAGO,0)
                                    WHEN COALESCE(COPAPROPIO,0)=1 THEN 0
                                    ELSE COALESCE(CP_VLR_COPAGOS,0) END,
             @DESCUENTO	     = DESCUENTO,
   ...
   BEGIN
      ...
      BEGIN
        ...
        WHERE  FTR.N_FACTURA = @N_FACTURA
	      ...
	      UPDATE FTR SET CONTABILIZADA=2,NROCOMPROBANTE=@MAUX WHERE N_fACTURA=@N_FACTURA
         IF @ESTADO='A' 
         BEGIN 
   ...
   BEGIN
      ...
      -- DEDUCCIONES
      IF (@PROCEDENCIA IN ('FINANCIERO','CE','CI','ACTIVOS','CAJA','AUTPG')) --OR (@NOREFERENCIA='MASIVA')
      BEGIN
         PRINT 'Llenando Temporal para Deducciones FINANCIERO, CE, CI, ACTIVOS'
          PRINT @PROCEDENCIA
         -- bugfix/CESPER87 -- 2026-09-05 -- Si hay N_FACTURAPOS, no tratar como CE/CI puro
         SELECT @ESCOPAGOPOS = CASE WHEN EXISTS (
            SELECT 1 FROM FCJ WITH (NOLOCK)
            WHERE N_FACTURAPOS = @N_FACTURA AND ESTADO = 'P'
         ) THEN 1 ELSE 0 END
         /* bugfix/CESPER87 -- 2026-09-05
            N_FACTURAPOS = copago (complementa factura del cliente). N_FACTURA en FCJ = particular. */
         IF @PROCEDENCIA='CE' AND COALESCE(@ESCOPAGOPOS,0)=0
         BEGIN
            PRINT @PROCEDENCIA
            IF EXISTS (SELECT * FROM FTR WHERE N_FACTURA=@N_FACTURA AND NOREFERENCIA='CEUNIFICADO')
            BEGIN
               EXEC SPK_DIST_COPA_UNIFICA @N_FACTURA
            END   
                   ...
                   AUT.IDPLAN,AUT.IDAFILIADO,(CASE WHEN EXISTS(SELECT PAPELLIDO+' '+SAPELLIDO+' '+PNOMBRE+' '+SNOMBRE  --STORRES SE CAMBIA AUT.IDAFILIADO POR AFI.DOCIDAFILIADO
                                              ...
                                              WHERE AFI.IDAFILIADO=AUT.IDAFILIADO) THEN 
                                                (SELECT LEFT(PAPELLIDO+' '+SAPELLIDO+' '+PNOMBRE+' '+SNOMBRE,40) 
                                                 FROM AFI 
                                                 WHERE AFI.IDAFILIADO=AUT.IDAFILIADO) ELSE '' END), SUM(COALESCE(FTRD.VIVA,0)), 0,
                                                 NOADMISION=MAX(FTRD.NOADMISION),
            ...
            WHERE  FTR.N_FACTURA = @N_FACTURA
            GROUP BY TTEC.TIPOMOV, TTEC.CUENTA, AUT.IDPLAN,AUT.IDAFILIADO,
                     FTR.CUENTACXC, TTEC2.CUENTA, COALESCE(FTRD.CODUNG,''), COALESCE(FTRD.CODPRG,'') --STORRES SE AGREGA AFI.DOCIDAFILIADO
         END
         ELSE
            -- bugfix/CESPER87 -- 2026-09-05 -- Unificar caja si PROCEDENCIA=CAJA o copago N_FACTURAPOS
            IF @PROCEDENCIA='CI' OR @PROCEDENCIA='CAJA' OR COALESCE(@ESCOPAGOPOS,0)=1
            BEGIN
               PRINT @PROCEDENCIA
               IF EXISTS (SELECT * FROM FTR WHERE N_FACTURA=@N_FACTURA AND NOREFERENCIA='VARIOS')
               BEGIN
                  EXEC SPK_DIST_COPA_UNIFICA @N_FACTURA
               END   
               IF @PROCEDENCIA='CI' AND COALESCE(@ESCOPAGOPOS,0)=0
               BEGIN
                  INSERT INTO #T (TIPO, CUENTA, IDAREA, CCOSTO, VLR_TOTAL, VLR_SERVICI, VLR_COPAGOS, VLR_PAGCOMP, 
                             VLR_DESCUENTO, VLR_ABONOS, IDPLAN, IDAFILIADO, NOMBREAFILIADO, VLR_IVA, VLR_IMPUESTOS, NOINGRESO, 
                             CODUNG, CODPRG)
                  SELECT 'DB', ISNULL(ISNULL(FTR.CUENTACXC,TTEC.CUENTA),TTEC2.CUENTA), 
                          ...
                          SUM(FTRD.VLR_SERVICI) AS VLR_SERVICI, SUM(FTRD.VLR_COPAGOS) AS VLR_COPAGOS, 
                          SUM(FTRD.VLR_PAGCOMP) AS VLR_PAGCOMP, 0 AS VLR_DESCUENTO, 0 AS VLR_ABONOS,
                          CIT.IDPLAN,CIT.IDAFILIADO,(CASE WHEN EXISTS(SELECT TOP 1 PAPELLIDO+' '+SAPELLIDO+' '+PNOMBRE+' '+SNOMBRE  --STORRES SE CAMBIA AUT.IDAFILIADO POR AFI.DOCIDAFILIADO
                                                     FROM AFI 
                                                     WHERE AFI.IDAFILIADO=CIT.IDAFILIADO) THEN 
                                                       (SELECT LEFT(PAPELLIDO+' '+SAPELLIDO+' '+PNOMBRE+' '+SNOMBRE,40) 
                                                        FROM AFI 
                                                        WHERE AFI.IDAFILIADO=CIT.IDAFILIADO) ELSE '' END), SUM(COALESCE(FTRD.VIVA,0)), 0, 
                                                         NOADMISION=MAX(CIT.CONSECUTIVO),
                        MAX(COALESCE(FTRD.CODUNG,'')),MAX(COALESCE(FTRD.CODPRG,''))
                  ...
                  GROUP BY TTEC.TIPOMOV, TTEC.CUENTA, CIT.IDPLAN, CIT.IDAFILIADO,
                           FTR.CUENTACXC, TTEC2.CUENTA--, COALESCE ( FTRD.CODUNG,''), COALESCE( FTRD.CODPRG,'') --STORRES SE AGREGA AFI.DOCIDAFILIADO
               END
               ELSE
               BEGIN 
                  PRINT 'VOY A CONTABILIZAR EL RECIBO DE CAJA (copago N_FACTURAPOS)'
                  -- bugfix/CESPER87 -- 2026-09-05 -- Localizar recibo por N_FACTURAPOS; fallback CAJ+NOREFERENCIA
                  SELECT TOP 1 @CODCAJA = CODCAJA, @CNSFACJ = CNSFACJ
                  FROM FCJ WITH (NOLOCK)
                  WHERE N_FACTURAPOS = @N_FACTURA
                    AND ESTADO = 'P'
                  ORDER BY FECHA DESC

                  IF COALESCE(@CNSFACJ,'') = ''
                  BEGIN
                     SET @NOREFERENCIA=REPLACE(REPLACE(LTRIM(RTRIM(@NOREFERENCIA)),CHAR(10),''),CHAR(13),'')
                     SELECT TOP 1 @CODCAJA = C.CODCAJA
                     FROM CAJ C
                     WHERE @NOREFERENCIA LIKE RTRIM(C.CODCAJA) + '%'
                     ORDER BY LEN(RTRIM(C.CODCAJA)) DESC

                     IF COALESCE(@CODCAJA,'') <> '' AND LEN(@NOREFERENCIA) > LEN(RTRIM(@CODCAJA))
                     BEGIN
                        SET @CNSFACJ = SUBSTRING(@NOREFERENCIA, LEN(RTRIM(@CODCAJA)) + 1, 20)
                     END
                  END

                  IF COALESCE(@CNSFACJ,'') = ''
                  BEGIN
                     PRINT N'No se identifico el recibo de caja por N_FACTURAPOS ni por NOREFERENCIA. Continuo solo con la factura.'
                  END
                  ELSE
                  BEGIN
                     UPDATE MCPE SET REFERENCIA1=@CNSFACJ,REFERENCIA2=@CODCAJA WHERE NROCOMPROBANTE=@MAUX
                     UPDATE FCJ SET N_FACTURAPOS=@N_FACTURA,RECAUDO=1 WHERE CODCAJA=@CODCAJA AND CNSFACJ=@CNSFACJ

                     PRINT 'CONTABILIZO INGRESO A CAJA'
                     PRINT ' EXEC SPK_NC_CONTAB_CAJA_ING '+@CNSFACJ+','+@CODCAJA+','+@USUARIO+','+@SYS_COMPUTERNAME+','+@COMPANIA+','+@SEDE+','+@MAUX
                     EXEC DBO.SPK_NC_CONTAB_CAJA_ING @CNSFACJ,@CODCAJA,@USUARIO,@SYS_COMPUTERNAME,@COMPANIA,@SEDE,@MAUX
                     PRINT 'REGRESO A SEGUIR CON FACTURACION'
                  END
            ...
            BEGIN
               ...
               IF @CAPITADA = 0
               BEGIN
                  PRINT '-- Factura Financiera Evento'
                  ...
                  SELECT 'DB' TIPO, ISNULL(TTEC1.CUENTA,TTEC2.CUENTA), '', '', 0 AS VLR_TOTAL, 
                   SUM(FTRD.VLR_SERVICI) AS VLR_SERVICI, SUM(FTRD.VLR_COPAGOS) AS VLR_COPAGOS, 
                   SUM(FTRD.VLR_PAGCOMP) AS VLR_PAGCOMP, 0 AS VLR_DESCUENTO, 0 AS VLR_ABONOS, FTR.IDPLAN, SUM(COALESCE(FTRD.VIVA,0)), 0,MAX(FTRD.NOADMISION),
                   MAX(COALESCE ( FTRD.CODUNG,'')), MAX(COALESCE( FTRD.CODPRG,''))
                  FROM   FTR INNER JOIN FTRD ON FTR.CNSFCT    = FTRD.CNSFTR 
                             ...
                             LEFT JOIN  TTEC TTEC2 ON TTEC2.TIPO    = PPT.TIPOTERCONTABLE
                  WHERE  FTR.N_FACTURA = @N_FACTURA
                  GROUP BY ISNULL(TTEC1.CUENTA,TTEC2.CUENTA), FTR.IDPLAN
               ...
               BEGIN
                  ...
                  SELECT 'DB' TIPO, TTEC1.CUENTA, '', '', 0 AS VLR_TOTAL, 
                    SUM(FTRD.VLR_SERVICI) AS VLR_SERVICI, ROUND(CASE WHEN COALESCE(FTR.COPAPROPIO,0)=1 THEN CP_VLR_COPAGOS ELSE SUM(FTR.VALORCOPAGO)+COALESCE(FTR.VALORMODERADORA,0) END,0) VLR_COPAGOS, 
                  ...
                  BEGIN
                     IF COALESCE(@COPAPROPIO,0)=1
                     BEGIN
                        /* Copago propio digitado: no restar CP_VLR_COPAGOS de FTRD; CxC se fuerza a VR_TOTAL m�s adelante. */
                        PRINT 'COPAGO PROPIO + FACTURAPOS: no resto CP_VLR_COPAGOS de servicios FTRD'
                        UPDATE #T SET VLR_COPAGOS=0 WHERE 1=1
                        SELECT @DIFECOPAGOS=0, @VALORCOPAGO=0
                     END
                     ELSE
                     BEGIN
                  ...
                  BEGIN
                     /* Sin FACTURAPOS: el copago propio tampoco resta del detalle; CxC = VR_TOTAL (sin p�rdida). */
                     PRINT 'COPAGO PROPIO: no resto CP_VLR_COPAGOS de servicios FTRD'
                     UPDATE #T SET VLR_COPAGOS=0 WHERE 1=1
                     SELECT @DIFECOPAGOS=0, @VALORCOPAGO=0
                  END
               END
      ...
      /* COPAPROPIO: fuerza CxC = VR_TOTAL (equivalente a capita sin p�rdida; copago digitado no depende del recaudado). */
      IF @CAPITADA = 1 AND COALESCE(@COPAPROPIO,0) = 1
      BEGIN
         PRINT 'COPAGO PROPIO: fuerza VLR_TOTAL de CxC a VR_TOTAL de la factura'
         SELECT @VALORFTR = CASE COALESCE(PAQUETE,0) WHEN 0 THEN VR_TOTAL ELSE VALORSERVICIOS END
         FROM FTR WHERE N_FACTURA = @N_FACTURA
         SELECT @DIFECOPAGOS = 0, @VALORCOPAGO = 0
         UPDATE #T SET VLR_COPAGOS = 0
         IF (SELECT COUNT(*) FROM #T) = 1
         BEGIN
            UPDATE #T SET VLR_TOTAL = @VALORFTR
         END
         ELSE IF (SELECT SUM(ABS(COALESCE(VLR_TOTAL,0))) FROM #T) > 0
         BEGIN
            UPDATE #T SET VLR_TOTAL = ROUND(@VALORFTR * ABS(COALESCE(VLR_TOTAL,0)) / NULLIF((SELECT SUM(ABS(COALESCE(VLR_TOTAL,0))) FROM #T),0), 2)
            SELECT TOP 1 @CUENTA_T = CUENTA, @TIPO_T = TIPO, @IDAREA_T = IDAREA, @CCOSTO_T = CCOSTO
            FROM #T ORDER BY ABS(VLR_TOTAL) DESC
            SELECT @DIF = @VALORFTR - (SELECT SUM(VLR_TOTAL) FROM #T)
            UPDATE #T SET VLR_TOTAL = VLR_TOTAL + @DIF
            WHERE TIPO = @TIPO_T AND CUENTA = @CUENTA_T AND IDAREA = @IDAREA_T AND CCOSTO = @CCOSTO_T
         END
         ELSE
         BEGIN
            UPDATE #T SET VLR_TOTAL = @VALORFTR
         END
         PRINT '@VALORFTR COPAPROPIO CxC ='+STR(@VALORFTR)
      END
      ...
      SET @IDCATEGORIA = DBO.FNK_VALORVARIABLE('TERCATAFILIADO')      
	   ...
	   IF @PROCEDENCIA <> 'FINANCIERO'  AND   @PROCEDENCIA<>'INVENTARIO' AND @IDAFILIADO<>'MASIVA'
	   BEGIN
      ...
      BEGIN 
         ...
         PRINT 'COPAGOS...'
         IF @CAPITADA = 0 AND @PROCEDENCIA<>'AUTPG'
         BEGIN
         ...
         BEGIN
            ...
            BEGIN
               ...
               FROM   FTR INNER JOIN FTRD ON FTR.N_FACTURA=FTRD.N_FACTURA
                          LEFT JOIN AFI ON FTRD.IDAFI=AFI.IDAFILIADO, KCDE
               WHERE  FTR.N_FACTURA = @N_FACTURA
               AND    KCDE.TIPO  = @TIPO_DED 
               AND    KCDE.IDMODELOCONT = @IDMODELOCONT
            ...
            WHILE @@FETCH_STATUS = 0    
            BEGIN 
               PRINT 'Inserta Tercero con SPK_INSERT_TERDEAFI '+@AFI_RUC+','+@IDCATEGORIA --STORRES 20260325  SE CORRIGE CURSOR YA QUE ENVIABA @IDAFILIADO Y ESTE DEBE ENVIAR ES LO CAPTURADO EN EL SELECT DEL CURSOR QUE ES @AFI_RUC
               EXEC SPK_INSERT_TERDEAFI @AFI_RUC, 'AFI' --STORRES 20260325  SE CORRIGE CURSOR YA QUE ENVIABA @IDAFILIADO Y ESTE DEBE ENVIAR ES LO CAPTURADO EN EL SELECT DEL CURSOR QUE ES @AFI_RUC
               FETCH NEXT FROM AFIRUC_CURSOR    
               INTO @AFI_RUC
            END
            CLOSE AFIRUC_CURSOR
            ...
            INSERT INTO MCHE(NROCOMPROBANTE, COMPANIA, TIPO, CUENTA, IDTERCERO, CCOSTO, IDAREA, DETALLE,
                            ...
                            CLASECONT, GENERA, N_FACTURA, F_FACTURAREF, F_VENCE, PROCEDENCIA, REFERENCIA_PRO, 
                            CODUNG, CODPRG)
            SELECT @MAUX, @COMPANIA, 'DB', KCDE.CUENTA, AFI.IDAFILIADO,
                   FTRD.CCOSTO, FTRD.AREAPRESTACION, 'COPAGOS.', ROUND(SUM(FTRD.VLR_COPAGOS),0), FTR.N_FACTURA, NULL, NULL, 
                   @USUARIO, DBO.FNK_FECHA_SIN_MLS(GETDATE()), 1, 'PAGCOMP','S', 'Movimiento',
                   FTRD.NOADMISION, DBO.FNK_FECHA_SIN_HORA(FTR.F_FACTURA), DBO.FNK_FECHA_SIN_HORA(FTR.F_VENCE), 'FACTURA', @N_FACTURA, 
                   FTR.CODUNG, FTR.CODPRG
               FROM   FTR INNER JOIN FTRD ON FTR.N_FACTURA=FTRD.N_FACTURA
                          LEFT JOIN AFI ON FTRD.IDAFI=AFI.IDAFILIADO, KCDE
               WHERE  FTR.N_FACTURA = @N_FACTURA
               ...
               AND    1=CASE WHEN DBO.FNK_VALORVARIABLE('FACTURAPOS_ACTIVA')='SI' THEN 0 ELSE 1 END
               AND    FTR.PROCEDENCIA='AUTPG'
              GROUP  BY KCDE.CUENTA, AFI.IDAFILIADO, FTR.IDTERCERO, 
                   FTRD.CCOSTO, FTRD.AREAPRESTACION, FTR.N_FACTURA, FTRD.NOADMISION, DBO.FNK_FECHA_SIN_HORA(FTR.F_FACTURA), DBO.FNK_FECHA_SIN_HORA(FTR.F_VENCE),
                   FTR.CODUNG, FTR.CODPRG, FTRD.PROCEDENCIA
            HAVING SUM(FTRD.VLR_COPAGOS)>0

			   UPDATE MCHE SET IDTERCERO=TER.IDTERCERO
			   FROM MCHE
			   LEFT JOIN AFI ON AFI.IDAFILIADO=MCHE.IDTERCERO
			   LEFT JOIN TER ON TER.NIT=AFI.IDAFILIADO
...
--query2
      ...
      BEGIN
         ...
         BEGIN
            ...
            SELECT @MAUX, @COMPANIA, 'CR', 
                    DBO.FNK_NC_CUENTA_KMCOM(@IDMODELOCONT,'VENTAS','CR',FTRD.PREFIJO,DBO.FNK_VALIDA_CCOSTO(FTRD.CCOSTO,FTR.IDSEDE),'S',FTRD.REFERENCIA,@PGP),
                    FTR.IDTERCERO, DBO.FNK_VALIDA_CCOSTO(FTRD.CCOSTO,FTR.IDSEDE), 
                    UPPER(SER.DESCSERVICIO)+' FACTURA No.'+FTRD.N_FACTURA, 
                   SUM(+ CASE WHEN DBO.FNK_VALORVARIABLE('IXCOUNTRY') = 'PERU' OR DBO.FNK_VALORVARIABLE('FACTURAPOS_ACTIVA') = 'SI'  THEN
                     COALESCE(FTRD.VLR_SERVICI,0) -COALESCE(FTRD.VIVA,0)-COALESCE(FTRD.VLR_COPAGOS,0)
                   ELSE
            ...
            GROUP BY DBO.FNK_NC_CUENTA_KMCOM(@IDMODELOCONT,'VENTAS','CR',FTRD.PREFIJO, FTRD.CCOSTO,'S',FTRD.REFERENCIA,@PGP),
                     ...
                     DBO.FNK_FECHA_SIN_HORA(FTR.F_FACTURA), DBO.FNK_FECHA_SIN_HORA(FTR.F_VENCE),
                     FTRD.CODUNG, FTRD.CODPRG, FTRD.NOPRESTACION,FTR.IDSEDE
         END
         ...
         BEGIN
            ...
            SELECT @MAUX, @COMPANIA, 'CR', 
                   CASE WHEN COALESCE(DBO.FNK_VALORVARIABLE('IDTERCEROINSTALADO'),'') <> ''
                        THEN
                            CASE WHEN COALESCE(FTRD.IDPROVEEDOR,'') = DBO.FNK_VALORVARIABLE('IDTERCEROINSTALADO') 
                                 THEN DBO.FNK_NC_CUENTA_KMCOM_IPS(@IDMODELOCONT,'VENTAS','CR',FTRD.PREFIJO, DBO.FNK_VALIDA_CCOSTO(FTRD.CCOSTO,FTR.IDSEDE),'S',FTRD.REFERENCIA)
                                 ELSE CASE WHEN @PROCEDENCIA='CAJA' THEN (SELECT CUENTA  FROM KCDE WHERE TIPO='FTR_COPAGOS_INGRESO')
                                             ELSE DBO.FNK_NC_CUENTA_KMCOM(@IDMODELOCONT,'VENTAS','CR',FTRD.PREFIJO, DBO.FNK_VALIDA_CCOSTO(FTRD.CCOSTO,FTR.IDSEDE),'S',FTRD.REFERENCIA,@PGP) END
                            END
                        ELSE DBO.FNK_NC_CUENTA_KMCOM(@IDMODELOCONT,'VENTAS','CR',FTRD.PREFIJO, DBO.FNK_VALIDA_CCOSTO(FTRD.CCOSTO,FTR.IDSEDE),'S',FTRD.REFERENCIA,@PGP)         
                   END,FTR.IDTERCERO,
                   ...
                   --     ELSE FTR.IDTERCERO 
                   --END, 
                   DBO.FNK_VALIDA_CCOSTO(FTRD.CCOSTO,FTR.IDSEDE), 
                   CASE WHEN @PROCEDENCIA<>'CAJA' THEN  UPPER(ISNULL(SER.DESCSERVICIO,PRE.NOM_PREFIJO)) ELSE LEFT(FTRD.ANEXO,500) END  +' FACTURA No.'+FTRD.N_FACTURA,
                   CASE WHEN DBO.FNK_VALORVARIABLE('IXCOUNTRY') = 'PERU' OR DBO.FNK_VALORVARIABLE('FACTURAPOS_ACTIVA') = 'SI'  THEN
                     COALESCE(FTRD.VLR_SERVICI,0) -COALESCE(FTRD.VIVA,0)-COALESCE(FTRD.VLR_COPAGOS,0)
                   ELSE
                     FTRD.VLR_SERVICI - (CASE WHEN FTRD.VIVA IS NULL THEN 0 ELSE CASE WHEN  DBO.FNK_VALORVARIABLE('BASE_IVA_SERVICIOS')='SINIVA' THEN 0 ELSE FTRD.VIVA END END) 
                   END,
                   FTRD.REFERENCIA, FTRD.NOPRESTACION,CASE WHEN COALESCE(FTRD.VIVA,0)>0 THEN 'IVA' ELSE NULL END, FTRD.NOITEM, @USUARIO, 
      ...
      ELSE -- CAPITADA = 1
      BEGIN
         /* COPAPROPIO digitado: misma ruta de cr�ditos que CAPITA_SIN_PERDIDA (tope VR_TOTAL, sin PyG por diferencia de recaudado). */
         IF DBO.FNK_VALORVARIABLE('CAPITA_SIN_PERDIDA')<>'SI' AND COALESCE(@COPAPROPIO,0)<>1
         BEGIN
            ...
            FROM
                  (SELECT NROCOMPROBANTE=@MAUX, COMPANIA=@COMPANIA, TIPO='CR', 
                          CUENTA=DBO.FNK_NC_CUENTA_KMCOM(@IDMODELOCONT,'VENTAS','CR',FTRDC.PREFIJO,DBO.FNK_VALIDA_CCOSTO(FTRDC.CCOSTO,FTRDC.IDSEDE),'S','',@PGP),
                          FTR.IDTERCERO,DBO.FNK_VALIDA_CCOSTO(FTRDC.CCOSTO,FTRDC.IDSEDE)CCOSTO, DETALLE=UPPER(PRE.NOM_PREFIJO)+' FACTURA No.'+@N_FACTURA, VALOR=FTRDC.VALORTOTAL, 
         ...
         BEGIN
            ...
            IF COALESCE(@COPAPROPIO,0)=1
               PRINT 'COPAGO PROPIO: ruta de cr�ditos equivalente a CAPITA_SIN_PERDIDA'
			EXEC SPK_NC_SUMA_DBCR @MAUX
            ...
            SELECT  @VALORFTR=CASE WHEN COALESCE(@COPAPROPIO,0)=1 OR DBO.FNK_VALORVARIABLE('FACTURAPOS_ACTIVA')='SI'
                                   THEN VR_TOTAL ELSE VALORSERVICIOS END
            FROM FTR WHERE N_FACTURA=@N_FACTURA
            ...
            FROM
                  (SELECT NROCOMPROBANTE=@MAUX, COMPANIA=@COMPANIA, TIPO='CR', 
                          CUENTA=DBO.FNK_NC_CUENTA_KMCOM(@IDMODELOCONT,'VENTAS','CR',FTRDC.PREFIJO, DBO.FNK_VALIDA_CCOSTO(FTRDC.CCOSTO,FTRDC.IDSEDE),'S','',@PGP),
                          FTR.IDTERCERO,DBO.FNK_VALIDA_CCOSTO(FTRDC.CCOSTO,FTRDC.IDSEDE) CCOSTO, DETALLE=UPPER(PRE.NOM_PREFIJO)+' FACTURA No.'+@N_FACTURA, 
   ...
   END
   IF DBO.FNK_VALORVARIABLE('CONTAB_GASTOVEN_UNI')='SI' AND @PROCEDENCIA='INVENTARIO'
   BEGIN
      PRINT 'LE AGREGO EL COSTO DE INVENTARIO'
      EXEC SPK_NC_CONTAB_INV @NOREFERENCIA,@USUARIO,@SYS_COMPUTERNAME,@COMPANIA,@SEDE,@MAUX
   END

   PRINT 'VALIDO CUENTAS DE IVA.. Y REVERSION'

   PRINT 'VALIDO CUENTAS FINALES'
   ...
   BEGIN
      UPDATE MCHE SET CTAREVERSION=CASE WHEN MCHE.PROCEDENCIA='INV' THEN MCHE.CUENTA ELSE  CUENTAREV END
      FROM MCHE INNER JOIN CUE ON MCHE.CUENTA=CUE.CUENTA  
            WHERE MCHE.NROCOMPROBANTE=@NROCOMPROBANTE
            AND COALESCE(CUE.MCUEREV,0)=1
   END
   EXEC SPK_NC_REVISAMCPE @MAUX, @COMPANIA, @USUARIO, @SEDE, @SYS_COMPUTERNAME   
   -- bugfix/CESPER87 -- 2026-09-05 -- Mismo NROCOMPROBANTE en FCJ si copago o procedencia CAJA
   IF EXISTS(SELECT * FROM MCP WHERE NROCOMPROBANTE=@MAUX)
      AND ( @PROCEDENCIA='CAJA' OR COALESCE(@ESCOPAGOPOS,0)=1 OR COALESCE(@CNSFACJ,'')<>'' )
   BEGIN
      IF COALESCE(@CNSFACJ,'')<>''
      BEGIN
         UPDATE FCJ SET CONTABILIZADA=1,NROCOMPROBANTE=@MAUX WHERE CODCAJA=@CODCAJA AND CNSFACJ=@CNSFACJ
      END
      ELSE
      BEGIN
         UPDATE FCJ SET CONTABILIZADA=1,NROCOMPROBANTE=@MAUX WHERE N_FACTURAPOS=@N_FACTURA AND ESTADO='P'
      END
   END
