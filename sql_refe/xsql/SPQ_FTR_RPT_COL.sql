	...
	BEGIN         
		...
		BEGIN
			...
			SELECT @SQL=
						...
						CASE WHEN FTR.ESTADO	= ''P'' AND COALESCE (FTR.TIPOANULACION, '''') = '''' THEN	''Activa''
						        WHEN FTR.ESTADO	= ''P'' AND COALESCE (FTR.TIPOANULACION, '''') = ''NC'' THEN	''Anulada Por NC''
								WHEN FTR.ESTADO	= ''A'' AND COALESCE (FTR.TIPOANULACION, '''') = '''' THEN	''Anulada''
								END ESTADO,
						CASE COALESCE(FTR.FACTEP,'''') WHEN '''' THEN ''Sin Notificar'' WHEN ''101'' THEN ''En Proceso'' WHEN ''102'' THEN ''Aceptado''
							WHEN ''103'' THEN ''Aceptado Con Observacion'' WHEN ''104'' THEN ''Rechazado Por Sunat'' WHEN ''105'' THEN ''Anulado''
							WHEN ''108'' THEN ''En Solicitud De Baja No Enviado A Sunat'' ELSE COALESCE(FTR.FACTEP,'''') END ESTADO_SUNAT
			...
			SELECT PLN.DESCPLAN,DBO.FNK_FECHA_GRINGA(FNOT.F_NOTA)F_NOTA,DBO.FNK_FECHA_GRINGA(FNOT.F_NOTA)F_NOTA,CASE FTR.TIPOFIN WHEN ''N'' THEN IIF(FNOT.CLASE=''C'',''07'',''07'') WHEN ''C'' THEN IIF(FNOT.CLASE=''C'',''07'',''07'') END TIPO,
						...
						CASE WHEN FNOT.ESTADO = ''A'' THEN ''Anulada'' WHEN FNOT.ESTADO = ''O'' THEN ''Aplicada'' ELSE FNOT.ESTADO END ESTADO,
						CASE COALESCE(FNOT.FACTEP,'''') WHEN '''' THEN ''Sin Notificar'' WHEN ''101'' THEN ''En Proceso'' WHEN ''102'' THEN ''Aceptado''
							WHEN ''103'' THEN ''Aceptado Con Observacion'' WHEN ''104'' THEN ''Rechazado Por Sunat'' WHEN ''105'' THEN ''Anulado''
							WHEN ''108'' THEN ''En Solicitud De Baja No Enviado A Sunat'' ELSE COALESCE(FNOT.FACTEP,'''') END ESTADO_SUNAT
		...
		BEGIN
			...
			BEGIN
				SELECT @SQL=
					'SELECT FTR.IDTERCERO, TER.NIT,TER.RAZONSOCIAL,FTR.IDPLAN,PLN.DESCPLAN,
					(SELECT STRING_AGG(CAST(Z.AREAPRESTACION AS VARCHAR(20)), ''; '')
					 WITHIN GROUP (ORDER BY Z.AREAPRESTACION)
					 FROM (
					   SELECT DISTINCT FTRD.AREAPRESTACION
					   FROM FTR FTRX
					   INNER JOIN FTRD ON FTRD.N_FACTURA = FTRX.N_FACTURA
					   WHERE FTRX.IDTERCERO = FTR.IDTERCERO
					     AND COALESCE(FTRX.IDPLAN,'''') = COALESCE(FTR.IDPLAN,'''')
					     AND FTRX.ESTADO = FTR.ESTADO
					     AND COALESCE(FTRX.TIPOANULACION,'''') = COALESCE(FTR.TIPOANULACION,'''')
					     AND FTRX.N_FACTURA NOT IN (SELECT N_FACTURA FROM #FTR)
					     AND FTRX.F_FACTURA >= '''+CONVERT(VARCHAR(10),@F_INICIAL,103)+'''
					     AND FTRX.F_FACTURA <  '''+CONVERT(VARCHAR(10),@F_FINAL + 1,103)+'''
					     AND COALESCE(FTRX.IDSEDE,'''') = CASE  WHEN '''+COALESCE(@IDSEDEF,'Todas')+'''<>''Todas'' THEN '''+COALESCE(@IDSEDEF,'Todas')+''' ELSE COALESCE(FTRX.IDSEDE,'''') END
					     AND FTRX.PROCEDENCIA = CASE WHEN COALESCE('''+COALESCE(@PROCEDENCIA,'Todas')+''',''Todas'')=''Todas'' THEN FTRX.PROCEDENCIA ELSE '''+COALESCE(@PROCEDENCIA,'Todas')+''' END
					     AND COALESCE(FTRD.AREAPRESTACION, '''') <> ''''
					 ) Z) AS AREAPRESTACION,
					(SELECT STRING_AGG(CAST(Z.DESCRIPCION AS VARCHAR(200)), ''; '')
					 WITHIN GROUP (ORDER BY Z.AREAPRESTACION)
					 FROM (
					   SELECT DISTINCT FTRD.AREAPRESTACION, COALESCE(AFU.DESCRIPCION, '''') DESCRIPCION
					   FROM FTR FTRX
					   INNER JOIN FTRD ON FTRD.N_FACTURA = FTRX.N_FACTURA
					...
					COUNT(*) CANTIDAD, SUM(FTR.VALORSERVICIOS) VALOR_SERVICIOS, SUM(FTR.VALORCOPAGO) VALORCOPAGO, 
					SUM(FTR.VALORPCOMP) VALOR_PCOMP, SUM(FTR.DESCUENTO) DESCUENTO, SUM(FTR.VALORSERVICIOS)-SUM(FTR.DESCUENTO)-SUM(FTR.VALORPCOMP)-SUM(FTR.VALORCOPAGO) VR_TOTAL,
					SUM(CASE WHEN  FTR.ESTADO=''P'' AND COALESCE(TIPOANULACION,'''')<>''NC'' THEN 1 ELSE 0 END) ACTIVAS,SUM(CASE WHEN  FTR.ESTADO=''A'' THEN 1 ELSE 0 END)ANULADAS,
					SUM( CASE WHEN COALESCE(FTR.TIPOANULACION,'''')=''NC'' THEN 1 ELSE 0 END) NOTACR,SUM(CASE COALESCE(FTR.FACTE,0) WHEN 0 THEN 1 ELSE 0 END) ENCOLA,
					SUM(CASE WHEN COALESCE(FTR.FACTE,0)= 1 THEN 1  ELSE 0 END) ENCOLA,SUM(CASE WHEN COALESCE(FTR.FACTE,0)=2 THEN 1 ELSE 0 END) ENVIADA,
				...
				SELECT @SQL=
					...
					(SELECT STRING_AGG(CAST(Z.AREAPRESTACION AS VARCHAR(20)), ''; '')
					 ...
					 FROM (
					   SELECT DISTINCT FTRD.AREAPRESTACION
					   FROM FTRD
					   WHERE FTRD.N_FACTURA = FTR.N_FACTURA
					     AND COALESCE(FTRD.AREAPRESTACION, '''') <> ''''
					 ) Z) AS AREAPRESTACION,
					(SELECT STRING_AGG(CAST(Z.DESCRIPCION AS VARCHAR(200)), ''; '')
					 WITHIN GROUP (ORDER BY Z.AREAPRESTACION)
					...
					FTR.N_FACTURA,CONVERT(VARCHAR,F_FACTURA,103)F_FACTURA,FTR.PROCEDENCIA,AFI.DOCIDAFILIADO,AFI.NOMBREAFI,
					FTR.VALORSERVICIOS,FTR.VALORCOPAGO AS VALORCOPAGO, 
					FTR.VALORPCOMP VALOR_PCOMP, FTR.DESCUENTO DESCUENTO, FTR.VALORSERVICIOS-FTR.DESCUENTO-FTR.VALORPCOMP-FTR.VALORCOPAGO VR_TOTAL,
					CASE WHEN FTR.ESTADO=''P'' AND COALESCE(TIPOANULACION,'''')<>''NC'' THEN ''ACTIVA'' ELSE ''ANULADA'' END ESTADO,
					CASE COALESCE(FTR.FACTE,0) WHEN 0 THEN ''SINENVIAR'' WHEN 1 THEN ''ENCOLA'' WHEN 2 THEN ''NOTIFICADA'' WHEN 3 THEN ''CONERROR''
		...
		BEGIN
			...
			SELECT @SQL=
			' SELECT FTRD.PREFIJO, COALESCE(PRE.NOM_PREFIJO,PLN.DESCPLAN) DESCRIPCION,FTRD.CCOSTO,COALESCE(CEN.DESCRIPCION,'''') DESCRIPCION_CCOSTO,SUM(COALESCE(FTRD.VLR_SERVICI,0)) VALORSERVICIOS,
			...
			GROUP BY  FTRD.PREFIJO, COALESCE(PRE.NOM_PREFIJO,PLN.DESCPLAN), FTRD.CCOSTO, COALESCE(CEN.DESCRIPCION,'''')'
			SELECT 'OK' OK
			EXECUTE(@SQL)
		END
		...
		BEGIN
			...
			SELECT @SQL=
			' SELECT FTRD.REFERENCIA,FTRD.ANEXO,FTRD.CCOSTO,COALESCE(CEN.DESCRIPCION,'''') DESCRIPCION_CCOSTO,SUM(FTRD.CANTIDAD)CANTIDAD,SUM(COALESCE(FTRD.VALOR,0)) VLR_UNITARIO,SUM(FTRD.CANTIDAD*COALESCE(FTRD.VALOR,1))VLR_BRUTO,
			SUM(CASE WHEN COALESCE(COPAGOS_CP,0)>0 THEN COALESCE(COPAGOS_CP,0) ELSE COALESCE(FTRD.VLR_COPAGOS,0) END) VLRCOPAGOS,
			SUM(FTRD.VLR_SERVICI-CASE WHEN COALESCE(COPAGOS_CP,0)>0 THEN COALESCE(COPAGOS_CP,0) ELSE COALESCE(FTRD.VLR_COPAGOS,0) END)VR_TOTAL , COALESCE(FTRD.N_FACTURA,'''') FACTURA, COALESCE(MED.NOMBRE, '''') AS PRESTADOR
			FROM FTRD  LEFT JOIN MED ON FTRD.IDPROVEEDOR=MED.IDMEDICO 
						LEFT JOIN CEN ON FTRD.CCOSTO=CEN.CCOSTO
			WHERE EXISTS('+
			' SELECT *  FROM   FTR                  WHERE  FTR.N_FACTURA NOT IN (SELECT N_FACTURA FROM #FTR)
					AND    F_FACTURA >= '''+CONVERT(VARCHAR(10),@F_INICIAL,103)+'''
			...
			GROUP BY  FTRD.REFERENCIA, FTRD.ANEXO , FTRD.CCOSTO, COALESCE(CEN.DESCRIPCION,''''), COALESCE(FTRD.N_FACTURA,''''), COALESCE(MED.NOMBRE, '''')
		...
		BEGIN
			...
			COALESCE(FTR.TIPOTTEC,''SINTTEC'')TIPOTTEC,COALESCE(TTEC.DETALLE,''NO DEFINIDO'')DETALLE,
			...
			CASE WHEN FTR.ESTADO=''P'' AND COALESCE(TIPOANULACION,'''')<>''NC'' THEN ''ACTIVA'' ELSE ''ANULADA'' END ESTADO,
			CASE COALESCE(FTR.FACTE,0) WHEN 0 THEN ''SINENVIAR'' WHEN 1 THEN ''ENCOLA'' WHEN 2 THEN ''NOTIFICADA'' WHEN 3 THEN ''CONERROR''
			WHEN 4 THEN ''NOTIFICADA'' ELSE ''OTRO'' END ESTADODIAN,FTR.CONTABILIZADA,FTR.NROCOMPROBANTE , IIF (FTR.PROCEDENCIA=''SALUD'',FTR.NOREFERENCIA, '''') AS NOADMISION 
			, FCXCD.CNSCXC, FDEP.NOMBRE AS DEPENDENCIA, HADM.FECHA AS FECHA_INGRESO, HADM.FECHAALTAMED AS FECHA_EGRESO
            , CASE WHEN FCXC.CNSCXC<>'''' THEN IIF(FCXC.INDRECIBIDO=1, ''CONFIRMADO'',''RADICADO'' ) END ESTADO_CARTERA
            FROM FTR 
                INNER JOIN TER ON FTR.IDTERCERO=TER.IDTERCERO
                LEFT  JOIN PLN ON FTR.IDPLAN=PLN.IDPLAN
                LEFT  JOIN AFI ON FTR.IDAFILIADO=AFI.IDAFILIADO
                ...
                LEFT  JOIN TTEC ON FTR.TIPOTTEC=TTEC.TIPO
                LEFT  JOIN SED ON FTR.IDSEDE=SED.IDSEDE
                LEFT  JOIN FCXCD ON FCXCD.N_FACTURA=FTR.N_FACTURA
                LEFT  JOIN FCXC ON FCXC.CNSCXC=FCXCD.CNSCXC
                LEFT  JOIN FDEP ON FDEP.IDDEP=FTR.IDDEP
                LEFT  JOIN HADM ON HADM.NOADMISION=FTR.NOREFERENCIA AND FTR.PROCEDENCIA=''SALUD''
            WHERE  FTR.N_FACTURA NOT IN (SELECT N_FACTURA FROM #FTR)
			AND    F_FACTURA >= '''+CONVERT(VARCHAR(10),@F_INICIAL,103)+'''
			AND    F_FACTURA <  '''+CONVERT(VARCHAR(10),@F_FINAL + 1,103)+'''
			AND    COALESCE(FTR.IDSEDE,'''') = CASE  WHEN '''+COALESCE(@IDSEDEF,'Todas')+'''<>''Todas'' THEN '''+COALESCE(@IDSEDEF,'Todas')+''' ELSE COALESCE(FTR.IDSEDE,'''') END
			...
			EXECUTE(@SQL)
		END
		IF UPPER(@PROCESO) ='CIRUGIA'
		BEGIN
			PRINT 'INGRESO A CIRUGIA'
			SELECT @SQL=
			' SELECT FTRD.REFERENCIA,SER.DESCSERVICIO,FTRD.CCOSTO,COALESCE(CEN.DESCRIPCION,'''') DESCRIPCION_CCOSTO,SUM(FTRD.CANTIDAD)CANTIDAD,SUM(COALESCE(FTRD.VALOR,0)) VLR_UNITARIO,SUM(FTRD.CANTIDAD*COALESCE(FTRD.VALOR,1))VLR_BRUTO,
			SUM(CASE WHEN COALESCE(COPAGOS_CP,0)>0 THEN COALESCE(COPAGOS_CP,0) ELSE COALESCE(FTRD.VLR_COPAGOS,0) END) VLRCOPAGOS,
			SUM(FTRD.VLR_SERVICI-CASE WHEN COALESCE(COPAGOS_CP,0)>0 THEN COALESCE(COPAGOS_CP,0) ELSE COALESCE(FTRD.VLR_COPAGOS,0) END)VR_TOTAL,  COALESCE(MED.NOMBRE, '''') AS PRESTADOR
			FROM FTRD INNER JOIN SER ON FTRD.IDCIRUGIA = SER.IDSERVICIO LEFT JOIN MED ON FTRD.IDPROVEEDOR=MED.IDMEDICO 
						LEFT JOIN CEN ON FTRD.CCOSTO=CEN.CCOSTO
			WHERE EXISTS('+
			' SELECT *  FROM   FTR                  WHERE  FTR.N_FACTURA NOT IN (SELECT N_FACTURA FROM #FTR)
					AND    F_FACTURA >= '''+CONVERT(VARCHAR(10),@F_INICIAL,103)+'''
					...
					' +@PART+@ESTFTR+
			' AND FTR.N_FACTURA=FTRD.N_FACTURA)
			AND SER.IDSERVICIO = CASE WHEN COALESCE('''+COALESCE(@IDSERVICIO,'')+''','''')='''' THEN SER.IDSERVICIO ELSE '''+COALESCE(@IDSERVICIO,'')+''' END 
			GROUP BY  FTRD.REFERENCIA, SER.DESCSERVICIO, FTRD.CCOSTO, COALESCE(CEN.DESCRIPCION,''''), COALESCE(MED.NOMBRE, '''') 
			ORDER BY FTRD.REFERENCIA'
			SELECT 'OK' OK
			EXECUTE(@SQL)
		END
