...
END
GO
CREATE OR ALTER PROCEDURE DBO.SPK_DEVUELVERECIB_CAJA 
@CODCAJA VARCHAR(4), 
@CNSFACJ VARCHAR(20),
@COMPANIA VARCHAR(2),
@SEDE VARCHAR(5) 
...
BEGIN
   ...
   --BEGIN
   --   RAISERROR('Recibo de Caja no Aplica Para Devolución',16,1)
   --   RETURN
   --END
   ...
   IF (SELECT COALESCE(TCNSFACJ,'') FROM FCJ WHERE CNSFACJ = @CNSFACJ   AND    CODCAJA = @CODCAJA ) = ''
   BEGIN
		UPDATE FCJ SET TCNSFACJ = NULL WHERE  CNSFACJ = @CNSFACJ   AND    CODCAJA = @CODCAJA
   END
   IF NOT EXISTS(SELECT * FROM TFCJ WHERE CNSFACJ IN( SELECT COALESCE(TCNSFACJ,CNSFACJ) FROM   FCJ 
   ...
   BEGIN
      ...
      SELECT COALESCE(TCNSFACJ,CNSFACJ), N_RECIBO, CLASE_FAC, NOADMISION, NOPRESTACION,
      ...
      SELECT @NVOCONSEC = COALESCE(TCNSFACJ,CNSFACJ) FROM FCJ WHERE  CNSFACJ = @CNSFACJ AND CODCAJA = @CODCAJA
