...
start "Qrys.Quatec SERVIDOR" cmd /k "%~dp0servir-app.bat"

echo Esperando servidor en puerto %PORT%...
powershell -NoProfile -ExecutionPolicy Bypass -Command ^
  "$ok=$false; for($i=0;$i -lt 30;$i++){ try { $r=Invoke-WebRequest -UseBasicParsing -TimeoutSec 2 'http://127.0.0.1:%PORT%/api/health'; if($r.StatusCode -eq 200){ $ok=$true; break } } catch {} Start-Sleep -Seconds 1 }; if(-not $ok){ Write-Host 'ERROR: el servidor no respondio en 30 s.' -ForegroundColor Red; Write-Host 'Revise la ventana Qrys.Quatec SERVIDOR.' -ForegroundColor Yellow; exit 1 }"
...
start "" "%URL%"
echo.
echo Listo. NO cierre la ventana "Qrys.Quatec SERVIDOR" mientras use la app.
